...
export function attachStudioBg3dEditorMiscHost(h) {
  const {
    ...
    bakePhysicsPreview, pausePhysicsWhenHidden, immersiveSceneActive, immersiveTransitionActive,
    ...
    fogSliderMax, primitiveById, customModelById, batchCandidatesByModelId, batchedNodeIds,
    renderSceneEntity, sharedCharacterSceneContent, shadowSceneBounds, shadowMapSize,
    keyShadowFit, fillShadowFit, webXrDisabledReason, sceneContent, mainCameraNearClip,
    mainCameraUp, applyLensShift, mainCameraNode, immersiveCameraNode,
    mainScenePresentationNode, commonOrbitControls, commitSharedCharacterTransform,
    effectiveSelectedSharedCharacter, effectiveSelectedSharedCharacterElementId,
    includeSharedCharactersInCapture, mayApplyEmptySharedStageMutation,
    selectSharedStageMutation, setSelectedSharedCharacterElementId,
    setSharedStageMaterializationKind, setSharedStageMutationKind,
    sharedCharacterCaptureElementIds, sharedCharacterCaptureReadiness,
    sharedCharacterGroundings, sharedCharacterPreviewOmissionCount, sharedCharacterReadyCount,
    ...
    setBabylonDiagnosticState, setImmersiveStagePlan,
  } = { ...R, ...h };
  ...
  h.runBabylonDiagnostic = runBabylonDiagnostic;
  ...
  h.selectSceneEntity = selectSceneEntity;
  ...
  h.updateModelCloneStatuses = updateModelCloneStatuses;
  ...
  h.startStudioBg3dWebXr = startStudioBg3dWebXr;
  const endStudioBg3dWebXr = () => {
    void webXrControllerRef.current?.end();
  };
  h.endStudioBg3dWebXr = endStudioBg3dWebXr;
}
